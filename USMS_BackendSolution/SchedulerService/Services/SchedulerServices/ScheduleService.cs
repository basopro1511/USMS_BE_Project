	...
	public class ScheduleService
	{
		...
		public async Task<APIResponse> UpdateSchedule(ScheduleDTO scheduleDto)
		{
			...
			try
			{
				...
				var existingSchedule = _scheduleRepository.GetScheduleById(scheduleDto.ScheduleId);
		...
		/// <summary>
		/// Phân công giáo viên vào lịch học
		/// </summary>
		/// <param name="scheduleId"></param>
		/// <param name="teacherId"></param>
		/// <returns></returns>
		public APIResponse AssignTeacher(AssignTeacherDTO assignTeacherDTO)
		{
			APIResponse response = new APIResponse();
			try
			{
				// Có cần kiểm tra user có phải là giáo viên không? Hay là cái list được get ra để chọn đã lọc lại trước rồi
				var schedule = _scheduleRepository.GetScheduleById(assignTeacherDTO.ScheduleId);
				if (schedule == null)
				{
					response.IsSuccess = false;
					response.Message = "Không tìm thấy lịch học!";
					return response;
				}
				var existingSchedules = _scheduleRepository.GetSchedulesByDateAndSlot(schedule.Date, schedule.SlotId);
				if (CheckTeacherConflict(existingSchedules, assignTeacherDTO.UserId))
				{
					response.IsSuccess = false;
					response.Message = "Giáo viên này đã được phân công vào buổi này!";
					return response;
				}
				schedule.TeacherId = assignTeacherDTO.UserId;
				if (!_scheduleRepository.UpdateSchedule(schedule))
				{
					response.IsSuccess = false;
					response.Message = "Lỗi hệ thống!";
				}
				response.IsSuccess = true;
				response.Message = "Phân công giáo viên thành công!";
			}
			catch (Exception ex)
			{
				response.IsSuccess = false;
				response.Message = $"Lỗi khi phân công giáo viên: {ex.Message}";
			}
			return response;
		}
		#endregion

		#region Unassign Teacher
		/// <summary>
		/// Hủy phân công giáo viên khỏi lịch học
		/// </summary>
		/// <param name="scheduleId"></param>
		/// <returns></returns>
		public APIResponse UnassignTeacher(int scheduleId)
		{
			APIResponse response = new APIResponse();
			try
			{
				...
				response.IsSuccess = true;
				response.Message = "Hủy phân công giáo viên thành công!";
			}
			catch (Exception ex)
			{
				response.IsSuccess = false;
				response.Message = $"Lỗi khi hủy phân công giáo viên: {ex.Message}";
			}
			return response;
		}
		#endregion

		#region Check Teacher Conflict
		/// <summary>
		/// Kiểm tra xung đột của giáo viên
		/// </summary>
		/// <param name="existingSchedules"></param>
		/// <param name="teacherId"></param>
		/// <returns></returns>
		private bool CheckTeacherConflict(List<Schedule>? existingSchedules, string teacherId)
		{
			return (existingSchedules != null &&
				existingSchedules.Any(es =>
										es.TeacherId == teacherId));
		}
		#endregion
	}
}

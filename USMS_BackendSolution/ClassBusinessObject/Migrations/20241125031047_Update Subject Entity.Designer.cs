// <auto-generated />
using System;
using ClassBusinessObject.AppDBContext;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace ClassBusinessObject.Migrations
{
    [DbContext(typeof(MyDbContext))]
    [Migration("20241125031047_Update Subject Entity")]
    partial class UpdateSubjectEntity
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.0")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("ClassBusinessObject.Models.ClassSubjects", b =>
                {
                    b.Property<int>("ClassSubjectId")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("ClassSubjectId"));

                    b.Property<string>("ClassId")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("SemesterId")
                        .IsRequired()
                        .HasMaxLength(4)
                        .HasColumnType("nvarchar(4)");

                    b.Property<string>("SubjectId")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.HasKey("ClassSubjectId");

                    b.HasIndex("SemesterId");

                    b.HasIndex("SubjectId");

                    b.ToTable("ClassSubjects");

                    b.HasData(
                        new
                        {
                            ClassSubjectId = 1,
                            ClassId = "SE1702",
                            CreatedAt = new DateTime(2024, 11, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            SemesterId = "SP25",
                            SubjectId = "PRM392"
                        },
                        new
                        {
                            ClassSubjectId = 2,
                            ClassId = "SE1702",
                            CreatedAt = new DateTime(2024, 11, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            SemesterId = "SP25",
                            SubjectId = "PRN231"
                        },
                        new
                        {
                            ClassSubjectId = 3,
                            ClassId = "SE1702",
                            CreatedAt = new DateTime(2024, 11, 25, 0, 0, 0, 0, DateTimeKind.Unspecified),
                            SemesterId = "SP25",
                            SubjectId = "MLN122"
                        });
                });

            modelBuilder.Entity("ClassBusinessObject.Models.Semesters", b =>
                {
                    b.Property<string>("SemesterId")
                        .HasMaxLength(4)
                        .HasColumnType("nvarchar(4)");

                    b.Property<DateOnly>("EndDate")
                        .HasColumnType("date");

                    b.Property<string>("SemesterName")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateOnly>("StartDate")
                        .HasColumnType("date");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.HasKey("SemesterId");

                    b.ToTable("Semesters");

                    b.HasData(
                        new
                        {
                            SemesterId = "SP25",
                            EndDate = new DateOnly(2025, 3, 24),
                            SemesterName = "Spring2025",
                            StartDate = new DateOnly(2025, 1, 5),
                            Status = 1
                        },
                        new
                        {
                            SemesterId = "SU25",
                            EndDate = new DateOnly(2025, 8, 11),
                            SemesterName = "Summer2025",
                            StartDate = new DateOnly(2025, 5, 8),
                            Status = 1
                        },
                        new
                        {
                            SemesterId = "FA25",
                            EndDate = new DateOnly(2025, 11, 25),
                            SemesterName = "Fall2025",
                            StartDate = new DateOnly(2025, 9, 5),
                            Status = 1
                        });
                });

            modelBuilder.Entity("ClassBusinessObject.Models.Subjects", b =>
                {
                    b.Property<string>("SubjectId")
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<bool>("IsAvailable")
                        .HasColumnType("bit");

                    b.Property<int>("NumberOfSlot")
                        .HasColumnType("int");

                    b.Property<string>("SubjectName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2");

                    b.HasKey("SubjectId");

                    b.ToTable("Subjects");

                    b.HasData(
                        new
                        {
                            SubjectId = "PRM392",
                            CreatedAt = new DateTime(2024, 11, 25, 10, 10, 46, 577, DateTimeKind.Local).AddTicks(3773),
                            Description = "Description",
                            IsAvailable = true,
                            NumberOfSlot = 20,
                            SubjectName = "Mobile Programing",
                            UpdatedAt = new DateTime(2024, 11, 25, 10, 10, 46, 578, DateTimeKind.Local).AddTicks(2769)
                        },
                        new
                        {
                            SubjectId = "PRN231",
                            CreatedAt = new DateTime(2024, 11, 25, 10, 10, 46, 578, DateTimeKind.Local).AddTicks(2957),
                            Description = "Description",
                            IsAvailable = true,
                            NumberOfSlot = 20,
                            SubjectName = "Building Cross-Platform Back-End Application With .NET",
                            UpdatedAt = new DateTime(2024, 11, 25, 10, 10, 46, 578, DateTimeKind.Local).AddTicks(2958)
                        },
                        new
                        {
                            SubjectId = "MLN122",
                            CreatedAt = new DateTime(2024, 11, 25, 10, 10, 46, 578, DateTimeKind.Local).AddTicks(2959),
                            Description = "Description",
                            IsAvailable = true,
                            NumberOfSlot = 16,
                            SubjectName = "Political economics of Marxism – Leninism",
                            UpdatedAt = new DateTime(2024, 11, 25, 10, 10, 46, 578, DateTimeKind.Local).AddTicks(2960)
                        });
                });

            modelBuilder.Entity("ClassBusinessObject.Models.ClassSubjects", b =>
                {
                    b.HasOne("ClassBusinessObject.Models.Semesters", "Semester")
                        .WithMany("ClassSubjects")
                        .HasForeignKey("SemesterId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ClassBusinessObject.Models.Subjects", "Subject")
                        .WithMany("ClassSubjects")
                        .HasForeignKey("SubjectId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Semester");

                    b.Navigation("Subject");
                });

            modelBuilder.Entity("ClassBusinessObject.Models.Semesters", b =>
                {
                    b.Navigation("ClassSubjects");
                });

            modelBuilder.Entity("ClassBusinessObject.Models.Subjects", b =>
                {
                    b.Navigation("ClassSubjects");
                });
#pragma warning restore 612, 618
        }
    }
}

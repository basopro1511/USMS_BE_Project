...
namespace UserService.Services.UserService
{
    public class UserService
    {
        ...
        private readonly IAuthorizationServicee _authorizationServicee;
        public UserService(IAuthorizationServicee authorizationServicee)
        {
            _userRepository = new UserRepository();
            _authorizationServicee = authorizationServicee;
        }
        /// <summary>
        /// Hashing password
        /// </summary>
        ...
        public APIResponse AddUser(AddUserDTO addUserDTO)
        {
            ...
            if (addUserDTO.RoleId == 5 && string.IsNullOrEmpty(addUserDTO.MajorId))
            {
                ...
                {
                    IsSuccess = false,
                    Message = "MajorId bắt buộc phải là RoleId = 5 (Student)."
                };
            }
            // Validate Email
            ...
            if (existingUser != null)
            {
                ...
                {
                    IsSuccess = false,
                    Message = "Người dùng với UserId đã tồn tại."
                };
            }
        ...
        public APIResponse UpdateStudentStatus(string userId, int status)
        {
            ...
            {
                aPIResponse.IsSuccess = false;
                aPIResponse.Message = "Trạng thái không hợp lệ. Trạng thái phải nằm trong khoảng từ 0 đến 3.";
                return aPIResponse;
            }
            UserDTO user = _userRepository.GetUserById(userId);
            ...
            {
                aPIResponse.IsSuccess = false;
                aPIResponse.Message = "Chỉ sinh viên mới có thể thay đổi trạng thái này.";
                return aPIResponse;
            }
            ...
            if (isUpdated)
            {
                ...
                {
                    case 0:
                        aPIResponse.Message = "Vô hiệu hóa học sinh thành công.";
                        break;
                    case 1:
                        aPIResponse.Message = "Đặt trạng thái người dùng đang theo học thành công.";
                        break;
                    case 2:
                        aPIResponse.Message = "Đặt trạng thái người dùng đang tạm hoãn thành công.";
                        break;
                    case 3:
                        aPIResponse.Message = "Đặt thạng thái người dùng đã tốt nghiệp thành công.";
                        break;
                }
            }
            else
            {
                aPIResponse.IsSuccess = false;
                aPIResponse.Message = "Cập nhật trạng thái học sinh thất bại.";
            }
            return aPIResponse;
        }
        /// <summary>
        ...
        public APIResponse UpdateInfor(string userId, UpdateInforDTO updateInfor)
        {
            ...
            if (isUpdated)
            {
                ...
                {
                    IsSuccess = true,
                    Message = "Người dùng cập nhật thành công."
                };
            }
            return new APIResponse
            {
